-- Mencegah script berjalan sebelum game sepenuhnya dimuat (Wajib untuk Auto Execute)
...
-- Pastikan LocalPlayer ada (Penting untuk Auto Execute)
...
local State = {
	...
	pickupFocusLuck = false,
	pickupMinLuck = 10,
	pickupFocusRarity = false,
	pickupRarities = {},
...
local Window = Library:CreateWindow({
	...
	Footer = "Mine a Mountain | Modified Script",
	AutoShow = true,
	NotifySide = "Right",
...
local function backpackCapacity()
	...
	if State.spoofCarryX8 then
		base *= 8
	elseif ownsGamepass("CarryKgPlus4") then
		base *= 4
	end
...
Connections.schedulerConn = Services.RunService.Heartbeat:Connect(function(deltaTime)
	...
	if State.autoPickupActive then
		...
		if State.pickupFocusLuck or State.pickupFocusRarity then
			filter = function(inst)
				local passLuck = not State.pickupFocusLuck
				if State.pickupFocusLuck then
					local luckPts = math.floor(crystalLuck(inst) * 100 + 0.5)
					passLuck = luckPts >= (State.pickupMinLuck or 10)
				end
				
				local passRarity = not State.pickupFocusRarity
				if State.pickupFocusRarity then
					local rarity = crystalRarity(inst)
					passRarity = State.pickupRarities and State.pickupRarities[rarity] == true
				end
				
				return passLuck and passRarity
			end
		end
		
		local ok, err = pcall(pickupStep, filter)
		if not ok then
...
do
	local function install()
		...
		PickupBox:AddToggle("PickupFocusLuck", {
			Text = "Only Pick Luck Crystals",
			Default = false,
			Callback = function(value)
				State.pickupFocusLuck = value
			end,
		})

		PickupBox:AddSlider("PickupMinLuck", {
			Text = "Min Luck",
			Default = 10,
			Min = 1,
			Max = 1000,
			...
			Compact = false,
			Callback = function(value)
				State.pickupMinLuck = value
			end,
		})

		PickupBox:AddToggle("PickupFocusRarity", {
			Text = "Only Pick Selected Rarities",
			Default = false,
			Callback = function(value)
				State.pickupFocusRarity = value
			end,
		})

		PickupBox:AddDropdown("PickupRarities", {
			Text = "Select Rarities",
			Values = { "Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythic", "Empyrean", "Pulsar", "Quasar" },
			Multi = true,
			Default = {},
			Callback = function(value)
				State.pickupRarities = value
			end,
		})
...
do
	local function install()
		...
		local SCAN_SPOTS = {}
		do
			local c = Config.MountainCenter
			local rings = {
				{ r = 0,    count = 1,  yOffsets = { 300 } },           -- Pusat Langit
				{ r = 400,  count = 6,  yOffsets = { 200, -100 } },     -- Area Dalam
				{ r = 800,  count = 8,  yOffsets = { 300, -100 } },     -- Area Tengah
				{ r = 1200, count = 10, yOffsets = { 200, 0 } }         -- Area Luar
			}
			
			for _, ring in ipairs(rings) do
				if ring.r == 0 then
					for _, yOff in ipairs(ring.yOffsets) do
						SCAN_SPOTS[#SCAN_SPOTS + 1] = CFrame.new(c + Vector3.new(0, yOff, 0), c)
					end
				else
					for i = 1, ring.count do
						local angle = (i / ring.count) * math.pi * 2
						local px = c.X + math.cos(angle) * ring.r
						local pz = c.Z + math.sin(angle) * ring.r
						local yOff = ring.yOffsets[(i % #ring.yOffsets) + 1]
						SCAN_SPOTS[#SCAN_SPOTS + 1] = CFrame.new(Vector3.new(px, c.Y + yOff, pz), c)
					end
				end
			end
			
			-- Titik terakhir mengembalikan karakter ke gunung utama agar di render
			SCAN_SPOTS[#SCAN_SPOTS + 1] = CFrame.new(c + Vector3.new(0, 50, 0), c)
		end
		...
		local function swingGap(tool)
			if State.fastDigActive then return 0.015 end
		...
		local function swing(part, model, center)
			...
			return pcall(function()
				local burst = SWING_BURST
				if State.spoofPowerX8 then burst = burst * 8 end
		...
		local function step(deltaTime)
			...
			if phase == "scan" then
				if not scanned then
					...
					if scanIndex <= #SCAN_SPOTS then
						local spot = SCAN_SPOTS[scanIndex]
						hold(spot)
						requestStream(spot.Position)
						statusText = string.format("Wide Scanning %d/%d", scanIndex, #SCAN_SPOTS)
			...
			if phase == "mine" then
				...
				if swingClock >= gap then
					local maxSwings = State.fastDigActive and 12 or 4
					swung = math.min(math.floor(swingClock / gap), maxSwings)
		...
		local function setActive(value)
			...
			task.delay(0.5, function()
				if active and not next(targets) then
		...
		FarmBox:AddToggle("SpoofPowerX8_Farm", {
			Text = "Spoof Gamepass x8 (Power)",
			Default = false,
			Callback = function(value)
				State.spoofPowerX8 = value
			end,
		})
	...
	local function install()
		...
		do
			...
			local rings = {
				{ r = 0,    count = 1,  yOffsets = { 300 } },           -- Pusat Langit
				{ r = 400,  count = 6,  yOffsets = { 200, -100 } },     -- Area Dalam
				{ r = 800,  count = 8,  yOffsets = { 300, -100 } },     -- Area Tengah (Tepi Gunung)
				{ r = 1200, count = 10, yOffsets = { 200, 0 } }         -- Area Luar
			}
			...
			-- TITIK TERAKHIR: Wajib kembali ke tengah gunung agar batu di-render ulang!
			SCAN_SPOTS[#SCAN_SPOTS + 1] = CFrame.new(c + Vector3.new(0, 50, 0), c)
		end
		...
		local function step(deltaTime)
			...
			swingClock += deltaTime
			local swingNeed = math.max(State.fastDigActive and 0.01 or 0.02, Farm.swingGap(heldPick) * (State.fastDigActive and 0.1 or 0.4))
			...
			if loot and loot.Parent then
				...
				holdAt(CFrame.new(spot + Vector3.new(0, COLLECT_LIFT, 0), spot), requestStream(spot) or spot)
				
				if canSwing then
					local maxSwings = State.fastDigActive and 5 or 1
					local swung = math.min(math.floor(swingClock / swingNeed), maxSwings)
					swingClock -= swung * swingNeed
		...
		MoneyBox:AddToggle("FastDig", {
			Text = "Fast Auto Dig (Super Speed)",
			Default = false,
			Callback = function(value)
				State.fastDigActive = value
			end,
		})

		MoneyBox:AddToggle("SpoofCarryX8", {
			Text = "Spoof Gamepass x8 (Capacity)",
			Default = false,
			Callback = function(value)
				State.spoofCarryX8 = value
			end,
		})
		
		MoneyBox:AddToggle("SpoofPowerX8", {
			Text = "Spoof Gamepass x8 (Power)",
			Default = false,
			Callback = function(value)
				State.spoofPowerX8 = value
...
do
	local function install()
		...
		ConfigGroupbox:AddLabel("All settings save automatically every 15s.", true)
...
-- ====== AUTO SAVE SYSTEM SEAMLESS ======
task.spawn(function()
	-- Beri jeda 2 detik agar karakter & UI benar-benar siap sebelum memuat settingan
	task.wait(2)
	
	local userAutoloadConfig = SaveManager:GetAutoloadConfig()
	if userAutoloadConfig and userAutoloadConfig ~= "" then
		pcall(function() SaveManager:Load(userAutoloadConfig) end)
	else
		pcall(function() SaveManager:Load("Universe_AutoSave") end)
	end

	-- Loop Auto-Save di latar belakang (Setiap 15 Detik)
	while task.wait(15) do
		if not getgenv().UniverseLoaded then break end
		pcall(function()
			SaveManager:Save("Universe_AutoSave")
			
			local current = SaveManager:GetAutoloadConfig()
...
local function cleanupAll()
	-- Save data terakhir tepat saat keluar game atau script dimatikan
	pcall(function()
		SaveManager:Save("Universe_AutoSave")
		local current = SaveManager:GetAutoloadConfig()
		if current and current ~= "" then
