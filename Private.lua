...
local function pickupCandidates(free, origin, filterFunc)
	...
	local function consider(child)
		...
		-- Jika sedang menggunakan filter custom (seperti Luck/Rarity), abaikan filter harga minimum
		if filterFunc then
			if not filterFunc(child) then
				return
			end
		else
			-- Jika mode normal, gunakan filter harga minimum
...
Connections.schedulerConn = Services.RunService.Heartbeat:Connect(function(deltaTime)
	...
	if State.autoPickupActive then
		...
		-- Jika Focus Luck atau Focus Rarity menyala, gunakan filter khusus
		if State.pickupFocusLuck or State.pickupFocusRarity then
			filter = function(inst)
				-- Cek persentase Luck jika fiturnya aktif
				...
				-- Cek tipe Rarity jika fiturnya aktif
				local passRarity = not State.pickupFocusRarity
				if State.pickupFocusRarity then
					local rarity = crystalRarity(inst)
					-- Pastikan array pickupRarities ada dan rarity-nya dicentang
					passRarity = State.pickupRarities and State.pickupRarities[rarity] == true
				end
				
				-- Crystal akan diambil jika lolos syarat yang diaktifkan
